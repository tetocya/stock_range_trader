...
class MeanReversionStrategy(Strategy):
    ...
    def prepare(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        complete_entry_data = result[
            ["close", "buy_threshold", "range_score", "adx"]
        ].notna().all(axis=1)
        ...
        result["mean_reversion_exit_condition"] = (
            result[["close", "sell_threshold"]].notna().all(axis=1)
            & (result["close"] >= result["sell_threshold"])
        )
    ...
    def generate_signal(
        self, row: pd.Series, position: PositionContext
    ) -> Signal:
        ...
        if np.isfinite(close) and close <= position.entry_price * (
            ...
            return Signal(
                SignalAction.SELL, signal_date, ExitReason.STOP_LOSS
            )
        if _condition_is_true(row.get("range_breakdown_exit_condition", False)):
            return Signal(
                SignalAction.SELL, signal_date, ExitReason.RANGE_BREAKDOWN
            )
        if position.holding_days >= self.max_holding_days:
            return Signal(
                SignalAction.SELL, signal_date, ExitReason.MAX_HOLDING_PERIOD
            )
        if _condition_is_true(row.get("mean_reversion_exit_condition", False)):
            return Signal(
                SignalAction.SELL, signal_date, ExitReason.MEAN_REVERSION
            )
