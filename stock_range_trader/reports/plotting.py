...
def plot_equity_curve(
    ...
    """Save strategy equity against a close-to-close buy-and-hold benchmark."""

    _require_columns(equity_curve, {"date", "total_equity"}, "equity chart")
    _require_columns(market_data, {"date", "close"}, "buy-and-hold chart")
    destination = _prepare_path(path)
    benchmark = initial_capital * market_data["close"] / market_data["close"].iloc[0]
...
def plot_drawdown(equity_curve: pd.DataFrame, path: PathLike) -> Path:
    ...
    axis.fill_between(equity_curve["date"], drawdown_pct, 0.0, color="firebrick", alpha=0.25)
...
def generate_all_plots(result: BacktestResult, output_dir: PathLike) -> dict[str, Path]:
    ...
    return {
        ...
        "drawdown": plot_drawdown(
            result.equity_curve, directory / "drawdown.png"
        ),
    }
