...
1. 対象期間に未検証の企業行動または`split_ratio != 1`があればExecutable処理を開始せず`unsupported`とする
...
- Theoretical Buy & Hold：企業行動が検出されない区間の初日と最終日のProvider報告終値を使う価格リターン。資金・単元株・残余現金・コスト・配当は反映しません
- Executable Buy & Hold：企業行動が検出されない区間でStrategyと同じ初期資金を使い、初日のProvider報告始値へBUY側SlippageとCommissionを適用し、設定単元株で買える最大株数を購入してProvider報告終値で時価評価します。`max_position_pct`、配当、税金、強制売却は含みません
...
`open/high/low/close/volume`は後方互換のSignal laneです。`signal_*`と`execution_*`を別列とし、企業行動のない対象区間では後者を約定シミュレーションへ使います。調整済みSignal価格をFill、購入可能株数、手数料、現金残高に使用しません。当日終値のSignalは引き続き次に現れる営業日のExecution Openでのみ約定します。
...
yfinanceは選択期間に`Stock Splits != 0`が1件でもあれば、区間全体のStrategy／Executable Buy & Holdを`UnsupportedCorporateActionError`で停止します。Yahooの`Open/High/Low/Close/Volume`は`auto_adjust=False`で取得したProvider報告値ですが、歴史的な未調整basisだとは仮定しません。J-Quantsも選択期間に`AdjFactor != 1`があれば停止します。価格系列のbasisを実データで確認できるまで、Engine、Portfolio、Benchmarkのいずれも分割比率を株数へ適用しません。
...
- J-QuantsとYahooは調整規約や日付、出来高、売買代金が完全一致するとは限りません。J-Quantsの`AdjFactor != 1`またはyfinanceの`Stock Splits != 0`がある期間はExecutableバックテストをUnsupportedとします。比較Warningは品質調査の入口であり、正しい値の自動決定ではありません。
...
- yfinanceの明示的な株式分割・併合は現在、株数と取得原価へ反映しません。対象期間に存在すればExecutable結果を停止します。端数株のCash-in-lieu、配当再投資、配当税、銘柄コード変更、上場廃止、売買停止も再現しません。
- Theoretical Buy & Holdは資金制約、単元株、残余現金、配当、手数料、税金を含みません。Executable Buy & Holdは企業行動のない区間だけを対象に初回BUYの単元株、残余現金、Slippage、Commissionを含めますが、配当・税金・売却費用は含みません。
