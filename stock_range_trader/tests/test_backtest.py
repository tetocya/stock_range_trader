...
def test_signals_execute_only_at_the_following_bar_open() -> None:
    ...
    assert list(result.order_log["status"]) == ["filled", "filled"]
    assert (result.order_log["reason"] == "none").all()
...
def test_last_bar_signal_remains_unexecuted_without_fabricated_price() -> None:
    ...
    final_order = result.order_log.iloc[-1]
    assert final_order["status"] == "canceled"
    assert final_order["reason"] == "no_next_bar"
    assert pd.isna(final_order["scheduled_execution_date"])
    assert pd.isna(final_order["execution_price"])


def test_last_bar_buy_is_canceled_without_a_next_bar() -> None:
    frame = _market(open_prices=[100.0], closes=[84.0])
    ...
    order = result.order_log.iloc[0]
    assert order["side"] == "buy"
    assert order["status"] == "canceled"
    assert order["reason"] == "no_next_bar"
...
def test_buy_is_skipped_when_one_lot_exceeds_position_limit() -> None:
    ...
    assert order["reason"] == "insufficient_capital_for_lot"
    assert order["requested_shares"] == 0
    assert order["filled_shares"] == 0


def test_zero_volume_scheduled_bar_cancels_order_without_fill_or_carry() -> None:
    frame = _market(open_prices=[100.0, 90.0, 90.0], closes=[84.0, 100.0, 100.0])
    ...
    order = result.order_log.iloc[0]
    assert order["scheduled_execution_date"] == frame.loc[1, "date"]
    assert order["status"] == "canceled"
    assert order["reason"] == "non_tradable_bar"
    assert order["raw_open_price"] == 90.0
    assert pd.isna(order["execution_price"])


def test_drawdown_halt_rejects_new_buy_and_records_risk_limit() -> None:
    frame = _market(
        open_prices=[100.0, 100.0, 100.0, 100.0],
        closes=[84.0, 40.0, 84.0, 100.0],
    )
    engine = BacktestEngine(
        strategy=MeanReversionStrategy(),
        execution_model=MarketOnNextOpen(slippage_pct=0.0),
        risk_manager=RiskManager(max_drawdown_stop=0.05),
        initial_capital=1_000_000.0,
    )

    result = engine.run("7203", frame)

    rejected = result.order_log.loc[result.order_log["reason"] == "risk_limit"]
    assert len(rejected) == 1
    assert rejected.iloc[0]["status"] == "rejected"
    assert rejected.iloc[0]["side"] == "buy"
    assert rejected.iloc[0]["filled_shares"] == 0
...
def test_result_writes_trade_and_equity_csv(tmp_path: Path) -> None:
    ...
    order_path = tmp_path / "nested" / "order_log.csv"
    result.save_order_log(order_path)
    result.save_equity_curve(equity_path)

    trade_csv = pd.read_csv(trade_path)
    order_csv = pd.read_csv(order_path)
    equity_csv = pd.read_csv(equity_path)
    assert list(trade_csv.columns) == list(result.trade_log.columns)
    assert len(trade_csv) == 1
    assert list(order_csv.columns) == list(result.order_log.columns)
    assert len(order_csv) == len(result.order_log)
