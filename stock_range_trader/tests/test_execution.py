...
from backtest import MarketOnNextOpen, Order, OrderSide, Portfolio
...
def test_buy_slippage_is_added_to_next_open() -> None:
    ...
    fill = model.execute(
        _order(OrderSide.BUY), pd.Timestamp("2025-01-07"), open_price=1_000.0
    )
...
def test_sell_slippage_is_subtracted_from_next_open() -> None:
    ...
    fill = model.execute(
        _order(OrderSide.SELL, reason=ExitReason.MEAN_REVERSION),
...
def test_portfolio_buy_and_sell_cash_flow_matches_net_profit() -> None:
    ...
    assert portfolio.cash == pytest.approx(
        portfolio.initial_capital + trade.net_profit
    )
...
def test_portfolio_mark_to_market_equity() -> None:
    portfolio = Portfolio(initial_capital=1_000_000.0)
    fill = MarketOnNextOpen(slippage_pct=0.0).execute(
        _order(OrderSide.BUY), pd.Timestamp("2025-01-07"), open_price=1_000.0
    )
...
def test_portfolio_rejects_sell_without_position() -> None:
    fill = MarketOnNextOpen().execute(
        _order(OrderSide.SELL, reason=ExitReason.STOP_LOSS),
        pd.Timestamp("2025-01-07"),
        open_price=1_000.0,
    )
...
def test_portfolio_rejects_second_long_position() -> None:
    ...
    portfolio = Portfolio()
    first = model.execute(
        _order(OrderSide.BUY), pd.Timestamp("2025-01-07"), 1_000.0
    )
