...
def validate_ohlcv(frame: pd.DataFrame) -> None:
    ...
    _validate_price_relationship(frame, frame["high"] < frame["open"], "high >= open")
    _validate_price_relationship(frame, frame["high"] < frame["close"], "high >= close")
    _validate_price_relationship(frame, frame["low"] > frame["open"], "low <= open")
    _validate_price_relationship(frame, frame["low"] > frame["close"], "low <= close")
...
def _format_bad_locations(mask: pd.DataFrame) -> str:
    ...
    locations = [
        ...
        for row, column in zip(*np.where(mask.to_numpy()), strict=True)
    ]
