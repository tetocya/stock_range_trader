...
class RangeDetector:
    ...
    def transform(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        result["rolling_high"] = (
            result["high"]
            .rolling(window=self.range_window, min_periods=self.range_window)
            .max()
        )
        result["rolling_low"] = (
            result["low"]
        ...
        result["rolling_mid"] = (result["rolling_high"] + result["rolling_low"]) / 2.0
        result["range_width"] = (
...
def mean_crossing_count(close: pd.Series, center: pd.Series, window: int) -> pd.Series:
    ...
    crossings = (side.ne(side.shift(1)) & side.notna() & side.shift(1).notna()).astype(
        float
    )
    crossings = crossings.where(side.notna())
    return (
        crossings.rolling(window=window, min_periods=window)
        .sum()
        .rename("ma_crossings")
    )
