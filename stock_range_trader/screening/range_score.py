...
class RangeScorer:
    ...
    median_trading_value_target: float = 100_000_000.0
    ...
    def __post_init__(self) -> None:
        positive_floats = {
            ...
            "median_trading_value_target": self.median_trading_value_target,
        }
    ...
    def transform(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        result["adx_score"] = _inverse_linear_score(result["adx"], self.adx_score_limit)
        result["trend_score"] = (
        ...
        atr_stability = _inverse_linear_score(result["atr_cv"], self.stability_cv_limit)
        ...
        result["liquidity_score"] = (
            100.0 * result["median_trading_value"] / self.median_trading_value_target
        ).clip(lower=0.0, upper=100.0)
        ...
        result["is_range_market"] = (result["range_score"] >= self.range_threshold) & (
...
def _rolling_coefficient_of_variation(values: pd.Series, window: int) -> pd.Series:
