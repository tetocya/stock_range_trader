...
class StrategyConfig:
    ...
    median_trading_value_target: float
    ...
    def from_mapping(cls, values: object) -> StrategyConfig:
        ...
        supplied = set(values)
        if "average_volume_target" in supplied:
            raise ValueError(
                "average_volume_target was retired in Phase 1.1; use "
        ...
        if missing:
            raise ValueError(
                "Missing strategy configuration keys: " + ", ".join(missing)
            )
        if unknown:
            raise ValueError(
                "Unknown strategy configuration keys: " + ", ".join(unknown)
            )
